const elements = {
   "header": "h3",
   "instruction": "p",
   "requiredLabel": ".ant-form-item-required", // это массив для лейблов, меняем только индекс в тесткейсах
   "name": "#name",
   "radioButtons": ".ant-radio",
   "radioButtonsLabel": ".ant-radio-wrapper",
   "age": "#age",
   "storyType": ".ant-select-selector",
   "storyList": ".ant-select-item",
   "imageLabel": ".ant-form-item-label",
   "image": "span.ant-upload",
   "submit": ".ant-btn",
   "tryAgain": ".btn-primary"


}
module.exports = elements; // позволяет видет файл за рамками этого файла
